...
public class CatzDriveStraight 
{
	...
	public static void PIDDriveNoTrig(double defaultPower, double distance, double timeoutSeconds) {
		...
		initalEncoderDistance = SmartDashboard.getNumber("encoder dist", 0.0);
		...
		while (done == false) {
			...
			currentDistance = Math.abs(Robot.driveTrain.getDriveTrainEncoderDistance());	
			...
			else 
			{
				...
				else 
				{
					 ...
					 * 
					 **************************************************************/
					Robot.driveTrain.arcadeDrive(drivePower, turnValue);
...
//			Timer.delay(PID_DRIVE_BRAKE_TIME);
		...
		if (defaultPower != 0.0) {
			...
			Robot.driveTrain.arcadeDrive(brakePower, 0.0);
			Timer.delay(0.070);
		...
		Robot.driveTrain.arcadeDrive(0.0, 0.0);    //Stop the robot
